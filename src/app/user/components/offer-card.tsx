...
import { MessageCircleMore, Clock, CheckCircle, XCircle, Trash2, Check, X, CircleCheck, ChevronsRight } from "lucide-react";
import Link from "next/link";
import toast from "react-hot-toast";
import SaleProfileCard from "./sale-profile-card";
...
const OfferCard: React.FC<OfferCardProps> = ({ offerDetail, onUpdate, isBuyer }) => {
  ...
  const handleCheckout = async () => {
    try {
      ...
      await checkout(offerDetail?.listing_id, me?.id, offerDetail.id); // 0 for no offerId
  ...
  return (
    ...
    <div className="flex w-full bg-white">
      ...
      <div className="flex-1 flex text-center justify-center items-center ">
        {offerDetail?.avaibility ?
          <span
            className={`inline-block px-2 py-0.5 text-xxxs font-medium rounded-full text-center ${statusStyles[offerDetail?.avaibility] || "bg-gray-100 text-gray-800"
              }`}
          >
            {offerDetail.avaibility.replace("_", " ").toUpperCase()}
          </span>
          : ""}
      ...
      <div className="flex-1 flex text-center justify-center items-center space-x-1">
        ...
        <p>
          <ChevronsRight size={10} color="gray" />
        </p>
        <p className="font-semibold text-black">
          {offerDetail.offered_price} ฿
        </p>
      </div>

      <div className="flex-1 flex text-center justify-center items-center space-x-2">
        ...
        {!isBuyer && offerDetail.status === "pending" && (
          <div className="flex space-x-2">
            <button
              ...
              className="group relative p-2 rounded-full bg-green-100 text-green-600 hover:bg-green-200 transition duration-150"
              title="Accept Offer"
            >
              <Check size={18} />
              <span className="absolute -top-7 left-1/2 transform -translate-x-1/2 bg-green-600 text-white text-xs rounded px-2 py-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                Accept
              </span>
            </button>
            ...
            <button
              ...
              className="group relative p-2 rounded-full bg-red-100 text-red-600 hover:bg-red-200 transition duration-150"
              title="Reject Offer"
            >
              <X size={18} />
              <span className="absolute -top-7 left-1/2 transform -translate-x-1/2 bg-red-600 text-white text-xs rounded px-2 py-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                Reject
              </span>
            </button>
        ...
        {isBuyer && offerDetail.status === "accepted" && offerDetail.avaibility === "for_sale" && (
          <button
            className="px-3 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-600 transition-all duration-200 ease-in-out transform hover:scale-105 active:scale-95 mx-2"
            title="Pay Now"
